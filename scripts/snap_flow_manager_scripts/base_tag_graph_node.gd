...
func _ready() -> void:
	...
	for child: Control in get_children():
		# Connect to signals # FIXME This is only for current tags at startup if tag added during session signals not connected!!
		if child.has_signal("remove_tag"):
			child.remove_tag.connect(handle_removing_tag)
			#print("connecting to remove tag signal from tag X button")
...
func _drop_data(position, data):
	...
	# Highlight tags that already exist and do not place. And add ones that do not exist.
	...
	else: # If current_tags doesn't have tag add it.
	...
	# Connect newly added tags X button "remove tag" signal.
	if tag.has_signal("remove_tag"):
		tag.remove_tag.connect(handle_removing_tag)
		#print("connecting to remove tag signal from tag X button")
...
# FIXME Tag not being removed from array that checks for already added tags in snap flow manager.
func handle_removing_tag(tag: Object) -> void:
	#var tag_text: String = tag.tag_line_edit.get_text()
	print("removing tag: ", tag.name)
	# Remove from current_tags when removed so that it can be re-added again.
	var tag_text: String = tag.get_meta("tag_text")
	if current_tags.has(tag_text):
		current_tags.erase(tag_text)
