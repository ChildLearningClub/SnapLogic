...
func _drop_data(position, data):
	...
	if current_tags.has(data.tag_line_edit.get_text()):
		for tag: Control in get_children():
			if tag.has_meta("tag_text") and tag.get_meta("tag_text") == data.tag_line_edit.get_text():
				highlight_tag(tag)
		return
	...
	tag.tag_line_edit.set_text(data.tag_line_edit.get_text())

	# Store the tag_text into the tag nodes metadata to make it persistent.
	tag.set_meta("tag_text", data.tag_line_edit.get_text())
